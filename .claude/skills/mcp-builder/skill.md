...
- **`models.py`**: Pydantic models, enums, and typed parameter classes for API request/response validation. Also include API payload serialization as methods on the relevant model (e.g. `to_payload()`) — keep this logic co-located with the model rather than as standalone functions in `utils.py`.
